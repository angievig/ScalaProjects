...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of aList with only one element (10pts)") {
    assert(sum(List(5)) == 5)
  }
  ...
  test("max of a few numbers (10pts)") {
   ...
   test("max of aList with only one element (10pts)") {
    assert(max(List(5)) == 5)
  }

   test("max of aList with two elements (10pts)") {
    assert(max(List(5,10)) == 10)
  }

   test("imax of of an empty List (10pts)") {
     try
       max(Nil)
       fail("No exception has been thrown")
     catch
